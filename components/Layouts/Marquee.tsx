...
const CarouselColumn = ({
  ...
  return (
    <div className="relative flex flex-col overflow-hidden flex-1 h-screen gap-2">
      {[animation, reverseAnimation].map((anim, animIndex) => (
        <div
          key={animIndex}
          className={`absolute w-full h-full flex flex-col gap-2 top-0 whitespace-nowrap ${anim}`}
        >
          {sizes.map((size, index) => (
            <div
              key={`${animIndex}-${index}`}
              className={`text-4xl block bg-gray-100/25 rounded-xl overflow-hidden relative text-${textColor[index]}`}
              style={{ height: size }}
            >
              <Image
                src={src[index]}
              ...
              {size === "50%" && (
                <>
                  <Plus
                    ...
                    size={30}
                  />
                  <p className="absolute bottom-2 left-5 text-base">
                    Texte ici
                  </p>
                </>
              )}
...
export default function Carousel() {
  const columns: CarouselColumnProps[] = [
    {
